...
{
    public partial class Despesas03 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Request.QueryString["Cliente"] != null)
            {
                if (!Page.IsPostBack)
                {
                    ...
                    string SelT = "select COALESCE(sum(pd_valor_previsto),0) as total from tb_despesas left join tb_cliente_despesas on da_id=pd_despesa where pd_cliente=@cliente and substring(da_codigo,1,2)='03' and  Length(da_codigo)>2";
                    ...
                    string Sel = "select * from (select pd_id,da_descricao,COALESCE(pd_valor_previsto,0) as pd_valor_previsto,da_id,da_codigo,da_formula from tb_despesas left join tb_cliente_despesas on da_id=pd_despesa where pd_cliente=@cliente or pd_id is null order by da_codigo) as b where substring(da_codigo,1,2)='03' and  Length(da_codigo)>2";
                    ...
                    con.Open();
                    string SelF = "select da_codigo,da_formula from tb_despesas where da_formula is not null and substring(da_codigo,1,2)='03' order by da_codigo";
