...
namespace Orcamento.Projeto.Fases.Profissionais
{
    public partial class Lista : System.Web.UI.Page
    {
        ...
        public void Carrega()
        {
            ...
            string Sel = "select pp_id,pp_projeto,pp_profissional,pp_hora_trabalhada,pp_quantidade,da_descricao from tb_projeto_profissional inner join tb_despesas on pp_profissional=da_id where pp_projeto=@projeto";
            MySqlCommand qrySelect = new MySqlCommand(Sel, con);
            qrySelect.Parameters.Add("@projeto", MySqlDbType.Int32).Value = Convert.ToInt32(lblProjeto.Text);
        ...
        protected void GrdProfissional_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandArgument.ToString()!="First" && e.CommandArgument.ToString()!="Next" && e.CommandArgument.ToString()!="Prev" && e.CommandArgument.ToString()!="Last")
            {
                if (Convert.ToInt16(e.CommandArgument.ToString())>=0)
                {
                    ...
                    {
                        Response.Redirect("~/Escritorio/Projeto/Fases/Profissionais/Cadastro.aspx?Projeto=" + lblProjeto.Text+"&ID="+id);
                    }
                    else
                    {
                        if (e.CommandName.CompareTo("Excluir") == 0)
                        {
                            Response.Redirect("~/Escritorio/Projeto/Fases/Profissionais/Excluir.aspx?Projeto=" + lblProjeto.Text+"&ID="+id);
                        }
                    }
                }
            }
        ...
        {
            Response.Redirect("~/Escritorio/Projeto/Criacao/Lista.aspx");
        }
        ...
        {
            Response.Redirect("~/Escritorio/Projeto/Fases/Profissionais/Cadastro.aspx?Projeto=" + lblProjeto.Text);
        }
    }
}