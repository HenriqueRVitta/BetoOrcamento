        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Request.QueryString["Cliente"] != null)
            {
                if (!Page.IsPostBack)
                {
                    ...
                    string SelT = "select COALESCE(sum(pd_valor_previsto),0) as total from tb_despesas left join tb_cliente_despesas on da_id=pd_despesa where pd_cliente=@cliente and substring(da_codigo,1,2)='06' and  Length(da_codigo)>2";
